...
def get_layer_id_for_vit(var_name, max_layer_id):
    ...
    if 'embedding' in var_name:
        return 0
    elif var_name.startswith('backbone.conv1'):
        return 0
    elif var_name.startswith('backbone.transformer.resblocks'):
...
class LearningRateDecayOptimizerConstructor(DefaultOptimWrapperConstructor):
    ...
    def add_params(self, params: List[dict], module: nn.Module,
        ...
        for name, param in module.named_parameters():
            ...
            if len(param.shape) == 1 or name.endswith('.bias') or (self.paramwise_cfg.get('no_decay_for_embedding', False) and 'embedding' in name): # NOTE 最后一个条件是我加的
                group_name = 'no_decay'
                this_weight_decay = 0.
            else:
                group_name = 'decay'
            ...
            if 'layer_wise' in decay_type:
                if 'MViT' in module.backbone.__class__.__name__:
                    ...
                    logger.info(f'MViT layer-wise: set param {name} as id {layer_id}')
                elif 'ViT' in module.backbone.__class__.__name__:
                    layer_id = get_layer_id_for_vit(
                        name, self.paramwise_cfg.get('num_layers'))
                    logger.info(f'ViT layer-wise: set param {name} as id {layer_id}')
                else:
