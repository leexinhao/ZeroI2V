...
class GradCAM:
    ...
    def _calculate_localization_map(self,
        ...
        if self.is_recognizer2d:
            ...
            if len(gradients.size()) == 3:
                import math
                print('Grad size:', gradients.size())
                print('Gctivations size: ', gradients.size())
                print("请确定你的模型是transformer，不要用最后一层attention之后的层！！！")
                gradients = gradients[1:, :, :] # 去掉cls token，不然没法看
                activations = activations[1:, :, :]
                HW, T, C = gradients.size()
                H = W = int(math.sqrt(HW))
                assert b == 1, b
                gradients = gradients.permute(2, 1, 0).reshape(1, C, T, H, W)
                activations = activations.permute(2, 1, 0).reshape(1, C, T, H, W)
