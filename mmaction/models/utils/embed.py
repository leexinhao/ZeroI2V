...
from mmengine.utils import to_2tuple, to_3tuple
...
class AdaptivePadding1d(nn.Module):
    ...
    def get_pad_shape(self, input_c):
        ...
        """
        output_c = math.ceil(input_c / self.stride)
        pad_c = max((output_c - 1) * self.stride +
                    (self.kernel_size - 1) * self.dilation + 1 - input_c, 0)
        return pad_c
    ...
    def forward(self, x):
        ...
        """
        pad_c = self.get_pad_shape(x.size()[-1])
        if pad_c > 0:
            if self.padding == 'corner':
                x = F.pad(x, [0, pad_c])
            elif self.padding == 'same':
                x = F.pad(x, [pad_c // 2, pad_c - pad_c // 2])
        return x


class AdaptivePadding2d(nn.Module):
    """Applies padding adaptively to the input.
    This module can make input get fully covered by filter
    you specified. It support two modes "same" and "corner". The
    "same" mode is same with "SAME" padding mode in TensorFlow, pad
...
class AdaptivePadding3d(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            x (Tensor): Input tensor has shape (B, C, T, H, W).
...
class PatchEmbed3D(BaseModule):
    ...
    def __init__(self,
        ...
        if isinstance(padding, str):
            self.adaptive_padding = AdaptivePadding3d(
                kernel_size=kernel_size,
                stride=stride,
                dilation=dilation,
                padding=padding)
