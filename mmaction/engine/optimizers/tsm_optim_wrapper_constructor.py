...
from mmengine.runner.checkpoint import print_log
import logging
            ...
            if isinstance(m, _ConvNd):
                ...
                if len(m_params) == 2:
                    normal_bias.append(m_params[1])
                            ...
                            (_BatchNorm, SyncBatchNorm_, torch.nn.GroupNorm, torch.nn.LayerNorm)): # noqa lxh: 为了兼容一些用ln的backbone
                ...
                if len(list(m.parameters())) > 0:
                    ...
                    print_log(f'New atomic module type: {type(m)}. Need to give it a learning policy', logger=logger, level=logging.WARNING)
                    # raise ValueError
