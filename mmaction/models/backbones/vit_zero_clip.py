...
class ResidualAttentionBlock_ZeroI2V(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1,
                 num_frames=8, dropout_rate=0.1, adapter_type='linear', mlp_ratio=0.25, stdha_cfg=dict()):
        super().__init__()
        self.num_tadapter = num_tadapter
        self.stdha_cfg = stdha_cfg
        self.attn = STDHA(embed_dim=d_model, num_heads=n_head, 
                num_frames=num_frames, shift_div=self.stdha_cfg.get('shift_div', 12),
...
class Transformer(nn.Module):
    def __init__(self, num_frames, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_tadapter=1, scale=1., dropout_rate=0.1, adapter_type='linear', mlp_ratio=0.25, stdha_cfg=dict()):
        super().__init__()
        self.width = width
        self.layers = layers
        # dpr = [x.item() for x in torch.linspace(0, drop_path, self.layers)]
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock_ZeroI2V(width, heads, attn_mask, scale, num_tadapter, num_frames, dropout_rate=dropout_rate, adapter_type=adapter_type, mlp_ratio=mlp_ratio, stdha_cfg=stdha_cfg) for i in range(layers)])
...
class ViT_Zero_CLIP(nn.Module):
    ...
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int,
     dropout_rate=0.1, num_tadapter=1, adapter_scale=0.5, pretrained=None, adapter_type='linear', mlp_ratio=0.25, stdha_cfg=dict(), final_ta_cfg=dict()):
        super().__init__()
        ...
        self.transformer = Transformer(num_frames, width, layers, heads, num_tadapter=num_tadapter, scale=adapter_scale, dropout_rate=dropout_rate, adapter_type=adapter_type, mlp_ratio=mlp_ratio, stdha_cfg=stdha_cfg)
