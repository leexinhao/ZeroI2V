...
class Resize(BaseTransform):
    ...
    def __init__(self,
        ...
        self.lazy = lazy
        self.must_divisible = must_divisible
    ...
    def transform(self, results):
        ...
        if self.keep_ratio:
            ...
            if self.must_divisible is not None:
                tmp_w = new_w % self.must_divisible
                if tmp_w != 0:
                    if (self.must_divisible - tmp_w) > tmp_w:
                        new_w -= tmp_w
                    else:
                        new_w += self.must_divisible - tmp_w
                tmp_h = new_h % self.must_divisible
                if tmp_h != 0:
                    if (self.must_divisible - tmp_h) > tmp_h:
                        new_h -= tmp_h
                    else:
                        new_h += self.must_divisible - tmp_h
        else:
...
class ThreeCrop(BaseTransform):
    ...
    def transform(self, results):
        ...
        crop_w, crop_h = self.crop_size
        assert crop_h == img_h or crop_w == img_w, f"{img_h} {img_w} {crop_h} {crop_w}"
